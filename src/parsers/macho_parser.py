...
def GetSymhash(binary):
  ...
  for symbol in binary.imported_symbols:
    ...
    is_stab = n_type & _N_STAB != 0
    is_external = n_type & _N_EXT == _N_EXT
    is_ntype = n_type & _N_TYPE != 0
...
def ParseCodeSignature(code_signature):
  ...
  if super_blob_magic == _CSMAGIC_EMBEDDED_SIGNATURE:
    ...
    for i in range(generic_blob_count):
      ...
      blob_index_offset = int.from_bytes(
        signature_bytes[start_index_entry_offset:start_index_entry_offset + 4], "big")
      ...
      if generic_blob_magic == _CSMAGIC_CODEDIRECTORY and blob_index_type == _CSSLOT_CODEDIRECTORY:
        ...
        cd_hash_calculated = GetDigest(sha256.SHA256Hasher(), code_directory)
        ...
        cd_hash_type = CSHashType(int.from_bytes(code_directory[37:38], "big"))
      ...
      elif generic_blob_magic == _CSMAGIC_BLOBWRAPPER and blob_index_type == _CSSLOT_SIGNATURESLOT:
...
def ParseMachoFatBinary(macho_fd, macho_path):
...
def ParseMachoBinary(macho_fd, binary):
  ...
  parsed_binary.segments = GetSegments(binary)
  parsed_binary.symbols = GetSymbols(binary)
  flags = []
  ...
  if binary.has_code_signature:
    parsed_binary.signature = ParseCodeSignature(binary.code_signature)
...
def parse_file(input_file, workflow_id) -> Report:
    """Parse a Mach-O file.
    - Parse Mach-O multi architecture files
    Args:
      input_file (file): Mach-O file.
    ...
    report = Report("Mach-O Parser Report")
    details_section = report.add_section()
    summary_section = report.add_section()
    report.summary = "No Mach-O content found"

    try:
      macho_path = input_file.get("path")
      ...
      if isinstance(macho_binary, lief.MachO.FatBinary):
        ...
        parsed_macho = ParsedMacho(
          signature=None, architecture=None, exports=None,
          fat_binary=None, arm64=None, x86_64=None)
        parsed_macho.workflow = workflow_id
        parsed_macho.source_path = macho_path
        parsed_macho.source_type = "file"
        parsed_macho.fat_binary = ParseMachoFatBinary(macho_fd, macho_path)
        for binary in macho_binary:
          parsed_binaries += 1
          parsed_binary = ParseMachoBinary(macho_fd, binary)
        ...
        parsed_macho.processing_time = CurrentTimeMillis() - start_time
        # Plain vanilla json.dumps() doesn't support custom classes.
        # https://stackoverflow.com/questions/3768895/how-to-make-a-class-json-serializable
        output = f'{json.dumps(parsed_macho.__dict__, default=lambda o: o.__dict__, indent=2)}\n'
        report.summary = f'Parsed {parsed_fat_binaries} lief.MachO.FatBinary and {parsed_binaries} lief.MachO.Binary'
      macho_fd.close()
    except IOError as e:
      report.summary = 'Error opening Mach-O file: {0:s}'.format(str(e))
